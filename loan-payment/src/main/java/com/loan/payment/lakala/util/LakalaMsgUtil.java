...
import com.loan_entity.lakala.webHook.SuperWebHookResponse;
...
public class LakalaMsgUtil {
     ...
     * @throws LakalaEncryptException
     */
    public static <T> LakalaCrossPayEncryptRequest encryptMsg(T obj, LakalaCrossPayEncryptRequest dataHead) throws LakalaEncryptException {
        ToolsUtil.remove();
        String publicKey = LakalaCrossPayEnv.getEnvConfig().getPublicKey();
        String privateKey = LakalaCrossPayEnv.getEnvConfig().getPrivateKey();
        String merId = LakalaCrossPayEnv.getEnvConfig().getMerId();
        ...
        try {
            ...
            throw new LakalaEncryptException("生成encKey失败", e);
        }
        Gson json = new Gson();
        ...
        try {
            ...
            throw new LakalaEncryptException("加密业务参数失败", e);
        }
        ...
        try {
            ...
            throw new LakalaEncryptException(e);
        }

        //组装加密请求参数
        LakalaCrossPayEncryptRequest req = new LakalaCrossPayEncryptRequest();
        req.setMerId(merId);
     ...
     * @param <T>        拉卡拉接口业务参数,扩展自父类LklCrossPaySuperRes
     * @return
     * @throws LakalaEncryptException
     */
    public static <T extends LakalaCrossPaySuperResponse> T decrypt(LakalaCrossPayEncryptResponse encryptRes, Class<T> resClazz) throws LakalaEncryptException {
        T result = null;
        ...
        try {
            ...
            reqMacStr = new String(RSAUtil.decryptByPublicKey(ByteArrayUtil.hexString2ByteArray(mac), LakalaCrossPayEnv.getEnvConfig().getPublicKey()));
        } catch (Exception e) {
            throw new LakalaEncryptException("mac解密失败", e);
        }
        //比较报文中mac sha1与本地mac sha是否相等
        //see if ur local mac is the same to mac from lakala
        if (!reqMacStr.equals(retMacStr)) {
            throw new LakalaEncryptException("mac校验失败");
        }
        //解密业务参数
        ...
        try {
            ...
            throw new LakalaEncryptException("业务参数解密失败", e);
        }
     ...
     * @return
     */
    public static <T extends SuperWebHookRequest> T decryptMsgFromLkl(LakalaCrossPayEncryptRequest lklCrossPayEncryptReq, Class<T> resClazz) {
        T result = null;
        ...
        String desKey = getMerchantKey(encKey, LakalaCrossPayEnv.getEnvConfig().getPrivateKey());
        ToolsUtil.remove();
     ...
     * @return
     */
    public static LakalaCrossPayEncryptResponse encryptWebHookMsg(SuperWebHookResponse webHookResponse, LakalaCrossPayEncryptResponse encryptRes) {
        String publicKey = LakalaCrossPayEnv.getEnvConfig().getPublicKey();
        String privateKey = LakalaCrossPayEnv.getEnvConfig().getPrivateKey();
...
//        encryptRes.setEncKey(encKey);
    ...
    private final static String getMerchantKey(String reqEncKey, String privateKey) {
        ...
        try {
            ...
            throw new LakalaEncryptException("解密请求方对称密钥失败", e);
        }
        return merKey.substring(merKey.length() - 32, merKey.length());
    }


    public static <T extends SuperWebHookRequest> T decryptMsgFromLklCustomAuth(LakalaCrossPayEncryptRequest lklCrossPayEncryptReq, Class<T> resClazz) {
        T result = null;
        String ts = lklCrossPayEncryptReq.getTs();
        String encData = lklCrossPayEncryptReq.getEncData();
        String reqType = lklCrossPayEncryptReq.getReqType();
    ...
    public static LakalaCrossPayEncryptResponse encryptWebHookMsgCustomAuth(SuperWebHookResponse webHookResponse, LakalaCrossPayEncryptResponse encryptRes) {
        String publicKey = LakalaCrossPayEnv.getEnvConfig().getPublicKey();
        String privateKey = LakalaCrossPayEnv.getEnvConfig().getPrivateKey();
